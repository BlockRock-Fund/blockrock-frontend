...
|---|---|---|
| **Who Benefits** | **How Decisions Get Made** | **How Work Gets Done** |
| Full-stack governance makes tokenholders first-class beneficiaries. | Decision markets prioritize performance. Skin-in-the-game replaces institutional politics. | AI enhances alpha generation. Funds scale with context and compute, not headcount. |
...
Dense multi-jurisdictional regulation hinders performance. Compliance layers delay action, fiduciary standards push for conservative allocations, and cross-border restrictions fragment strategy. The gap between how capital *should* move and how it *can* move is a compounding drag on returns.
...
> Fee models incentivize scale → Scale demands complexity → Complexity invites conservative compliance → Compliance slows decisions → Slow decisions reduce performance → Fees come in anyway.
...
Proof of safety: When MtnCapital wound down, holders received their proportional share of the treasury through the protocol's built-in liquidation mechanism. The system's guarantees worked as intended. Even in failure, no value was lost to extraction or mismanagement.
...
- **They scale with compute, not headcount.** As AI capabilities grow, the fund's capability grows too.
...
Token ownership → Better-incentivized market participants → More accurate futarchy → Better AI proposal filtering → Stronger performance → Higher token value → More sophisticated participants → Repeat

### The Resulting User Experience

| Passive Holders | Active Investors |
|---|---|
| Enjoy increasing treasury-backed value with secure structure, bullish decision-making, and minimal value leakage. | Submit proposals, trade decision markets, and profit for accurate judgment. |
